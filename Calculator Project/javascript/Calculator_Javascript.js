...
const Calculator = {
    // this is displays O on the screen
    Display_Value: '0',
  ...
  function Input_Digit(digit) {
    ...
    // this overwrites Display_Val ue if the current value is 0
...
function Handle_Operator(Next_Operator) {
    ...
    //checks if an operator already exists and if Wait Second_Operand is true,
  ...
  keys.addEventListener ('click', (event) => {
    ...
    // that was clicked
    const { target } = event;
    // if the element that was cli cked on is not a button, exit the f unction
